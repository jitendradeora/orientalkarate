...
import { Smile, Users, Award, Building2, ArrowRight, Sparkles } from "lucide-react";
        ...
        <div className="mx-auto grid max-w-7xl gap-8 px-4 sm:px-6 md:grid-cols-2 lg:px-8">
          {items.map((item, i) => (
            <motion.div
              ...
              className={`group relative overflow-hidden rounded-2xl border border-gray-200 bg-white p-8 shadow-sm transition-shadow hover:shadow-md`}
            >
              <div className={`pointer-events-none absolute inset-0 bg-gradient-to-br opacity-60 ${item.accent}`} />
              <div className="relative">
                <div className="mb-4 inline-flex rounded-2xl border border-gray-100 bg-gray-50 p-3">
                  <item.icon className="h-7 w-7 text-[#eb0339]" />
                </div>
                <p className="text-sm font-semibold text-[#eb0339]">{item.subtitle}</p>
                <h2 className="mt-1 text-2xl font-bold text-gray-900">{item.title}</h2>
                <p className="mt-3 text-sm leading-relaxed text-gray-600">{item.desc}</p>
                <Link
                  to={item.href}
                  className="mt-6 inline-flex items-center gap-2 rounded-full border border-gray-200 px-5 py-2 text-sm font-semibold text-gray-900 transition-all group-hover:border-[#eb0339] group-hover:text-[#eb0339] group-hover:gap-3"
                >
                  Learn more
                  <ArrowRight size={18} />
                </Link>
